...
def update_chart(entered_site, payload):
    if entered_site=='ALL':
        ...
        scatter_plot = px.scatter(filtered_df, x='Payload Mass (kg)',
                                y='class',
                                color='Booster Version Category')
        return scatter_plot
    else:
        selected_df=spacex_df[spacex_df['Launch Site']==entered_site]
        filtered_df=selected_df[(selected_df['Payload Mass (kg)']>=payload[0]) &
                                (selected_df['Payload Mass (kg)']<=payload[1])]
        scatter_plot = px.scatter(filtered_df, x='Payload Mass (kg)',
                                    y='class',
                                    color='Booster Version Category')
        return scatter_plot
